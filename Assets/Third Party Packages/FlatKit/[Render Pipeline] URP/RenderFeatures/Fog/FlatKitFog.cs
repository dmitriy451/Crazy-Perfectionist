...
namespace FlatKit
{
    public class FlatKitFog : ScriptableRendererFeature
    {
        ...
        private void InitMaterial()
        {
            ...
            if (_material == null) Debug.LogWarning("[FlatKit] Missing Fog Material");
        ...
        private void UpdateDistanceLut()
        {
            ...
            if (_lutDepth != null) DestroyImmediate(_lutDepth);
            ...
            {
                var color = settings.distanceGradient.Evaluate(x / (width - 1));
                for (float y = 0; y < height; y++) _lutDepth.SetPixel(Mathf.CeilToInt(x), Mathf.CeilToInt(y), color);
            }
        ...
        private void UpdateHeightLut()
        {
            ...
            if (_lutHeight != null) DestroyImmediate(_lutHeight);
            ...
            for (float x = 0; x < width; x++)
            {
                var color = settings.heightGradient.Evaluate(x / (width - 1));
                for (float y = 0; y < height; y++) _lutHeight.SetPixel(Mathf.CeilToInt(x), Mathf.CeilToInt(y), color);
            }
        ...
        private class EffectPass : ScriptableRenderPass
        {
            ...
            private readonly ProfilingSampler _profilingSampler = new("Flat Kit Fog");
