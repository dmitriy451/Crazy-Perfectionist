...
public class ScreenshotCompanion : MonoBehaviour
{
    ...
    public List<CameraObject> list = new();
    public SettingsaObject settings;


    [HideInInspector] public bool foldoutSettings;
    [HideInInspector] public int lastCamID;
    ...
    private void LateUpdate()
    {
        if (Input.anyKeyDown)
            ...
            {
                if ((int)list[i].hotkey == 0) continue;
                ...
                if (Input.GetKeyDown(list[i].hotkey.ToString().ToLower()))
                {
                    if (list[i] != null)
                    {
                        ...
                        {
                            var attachedCam = list[i].cam.GetComponent<Camera>();
                            if (attachedCam == null)
                    ...
                    {
                        DebugLogExtended("Screenshot by Hotkey (" + list[i].hotkey +
                                         ") could not be created! Camera not available.");
                    }
                }
            }
    }
    ...
    private void OnGUI()
    {
        ...
        if (lastP.x == settings.cutoutPosition.x && lastP.y == settings.cutoutPosition.y &&
            lastS.x == settings.cutoutSize.x && lastS.y == settings.cutoutSize.y)
        ...
        if (settings.captureMethod == CaptureMethod.Cutout)
        {
            ...
            GUI.Box(
                new Rect(settings.cutoutPosition.x - settings.cutoutSize.x / 2f,
                    settings.cutoutPosition.y - settings.cutoutSize.y / 2f, settings.cutoutSize.x,
                    settings.cutoutSize.y), "", cutoutBoxStyle);
        }
    }
    ...
    {
        var pickDirectory = settings.customDirectoryName != "" ? settings.customDirectoryName : "Screenshots";
    ...
    {
        settings.cutoutPosition.x = Mathf.Clamp(settings.cutoutPosition.x, settings.cutoutSize.x / 2f,
            Screen.width - settings.cutoutSize.x / 2f);
        settings.cutoutPosition.y = Mathf.Clamp(settings.cutoutPosition.y, settings.cutoutSize.y / 2f,
            Screen.height - settings.cutoutSize.y / 2f);

        settings.cutoutSize.x = Mathf.Clamp(settings.cutoutSize.x, 0f, Screen.width);
        settings.cutoutSize.y = Mathf.Clamp(settings.cutoutSize.y, 0f, Screen.height);
    }

    public void CaptureCutoutVoid(int id)
    {
        if (settings.singleCamera) activateCameraID(id);
    ...
    public IEnumerator CaptureCutout(int id)
    {
		...
		*/
        ...
        var directoryName = checkSaveDirectory();
        var fileName = directoryName + getFileName(id);
        ...
        DebugLogExtended("Captured newCutout Screenshot (" + width + "x" + height + " at " + startX + "," + startY +
                         ") saved to: " + fileName);
    ...
    private void cutoutEmptyCheck()
    {
        if (settings.cutoutSize.x <= 8f || settings.cutoutSize.y <= 8f)
        {
            ...
            else
            {
                ...
                settings.cutoutSize = new Vector2(Screen.width, Screen.height);
            }
        }
    }
    ...
    public void CaptureRenderTexture(Camera attachedCam, int id)
    {
        ...
        var resWidth = (int)(attachedCam.pixelWidth * settings.renderSizeMultiplier);
        var resHeight = (int)(attachedCam.pixelHeight * settings.renderSizeMultiplier);
    ...
    public string getFileName(int camID)
    {
        ...
        */
        ...
        if (fileName == "") fileName += DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss");

        // select filetype
        if (settings.fileType == FileType.JPG)
            fileName += ".jpg";
        else if (settings.fileType == FileType.PNG) fileName += ".png";
        ...
        if (lastCam == null || list[lastCamID].cam != lastCam.gameObject)
        {
            if (list[lastCamID].cam != null)
                ...
                for (var i = 0; i < list.Count; i++)
                {
                    ...
                    if (lastCam != null) break;
                }
        }

        if (lastCam == null) return "-x-";

        if (settings.captureMethod == CaptureMethod.RenderTexture)
            return (int)(lastCam.pixelWidth * settings.renderSizeMultiplier) + "x" +
                   (int)(lastCam.pixelHeight * settings.renderSizeMultiplier);

        return lastCam.pixelWidth * settings.captureSizeMultiplier + "x" +
               lastCam.pixelHeight * settings.captureSizeMultiplier;
    }
    ...
    {
        if (settings.includeCounter) settings.counter++;
    }
}