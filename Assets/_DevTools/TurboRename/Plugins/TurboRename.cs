...
public class TurboRename : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        numbermeth = (NumberedMethod)EditorGUILayout.EnumPopup(
            new GUIContent("Number method",
                "Number by position in selection, or number by hierarchy position. Note: Project files cannot be renamed with the hierarchy method as they are not present in the scene."),
            numbermeth);
        ...
        if (SelectedObjects.Length > 0)
        {
            ...
            if (showselection)
            {
                ...
                EditorGUILayout.Space();
                for (var i = 0; i < SelectedObjects.Length; i++) EditorGUILayout.LabelField(SelectedObjects[i].name);
                EditorGUILayout.EndVertical();
                EditorGUILayout.BeginVertical("Box");
                GUILayout.Label("Preview", EditorStyles.boldLabel);
                EditorGUILayout.Space();
                for (var i = 0; i < SelectedObjects.Length; i++) EditorGUILayout.LabelField(PreviewSelectedObjects[i]);
    ...
    private void Update()
    {
        ...
        for (var i = 0; i < SelectedObjects.Length; i++)
        {
            ...
            if (usenumbered && numbermeth == NumberedMethod.BySelection) str = str + (basenumbered + stepnumbered * i);
            ...
            if (usenumbered && numbermeth == NumberedMethod.ByHierarchy)
                for (var z = 0; z < SelectedGameObjectObjects.Length; z++)
                    if (SelectedGameObjectObjects[z] == SelectedObjects[i])
                        str = str + (basenumbered +
                                     stepnumbered * SelectedGameObjectObjects[z].transform.GetSiblingIndex());
    ...
    private void Rename()
    {
        for (var i = 0; i < SelectedObjects.Length; i++)
        {
            ...
            if (usenumbered && numbermeth == NumberedMethod.BySelection)
                SelectedObjects[i].name = SelectedObjects[i].name + (basenumbered + stepnumbered * i);

            if (useremove && remove != "") SelectedObjects[i].name = SelectedObjects[i].name.Replace(remove, "");
            if (usereplace && replace != "")
                SelectedObjects[i].name = SelectedObjects[i].name.Replace(replace, replacewith);
        ...
        for (var i = 0; i < SelectedGameObjectObjects.Length; i++)
            if (usenumbered && numbermeth == NumberedMethod.ByHierarchy)
                SelectedGameObjectObjects[i].name = SelectedGameObjectObjects[i].name +
                                                    (basenumbered + stepnumbered * SelectedGameObjectObjects[i]
