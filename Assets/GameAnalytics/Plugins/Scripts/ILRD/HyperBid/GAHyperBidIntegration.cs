...
public class GAHyperBidIntegration : MonoBehaviour
{
    ...
    public static void ListenForImpressions(Action<string> callback)
    {
        ...
        HyperBid.Api.HBInterstitialAd.Instance.events.onAdShowEvent +=
 (sender, args) => callback(args.callbackInfo.getOriginJSONString());
        HyperBid.Api.HBBannerAd.Instance.events.onAdImpressEvent +=
 (sender, args) => callback(args.callbackInfo.getOriginJSONString());
        HyperBid.Api.HBRewardedVideo.Instance.events.onAdVideoStartEvent +=
 (sender, args) => callback(args.callbackInfo.getOriginJSONString());
        HyperBid.Api.HBNativeAd.Instance.events.onAdImpressEvent +=
 (sender, args) => callback(args.callbackInfo.getOriginJSONString());
        _subscribed = true;
#endif
    }
